...
def getSegmentationMap(dfOuranno,serial_id,TOTAL_LEN):
    RDSegStr = dfOuranno.loc[serial_id,'RDSeg']
    RDSegStr = RDSegStr.strip(' ')
    #print(RDSegStr)
    segMap = [0]*TOTAL_LEN
    ...
    if(RDSegStr=='ALL'):
        ...
        segMap = np.array(segMap)
        return segMap
    if(RDSegStr!='NIL' and RDSegStr!='ALL'):
        segsplit = RDSegStr.split(';')
        numseg = len(segsplit)
        for i in range(numseg):
            myseg = segsplit[i].strip(' ')
            if(len(myseg)!=0):
                startEnd = myseg.split(':')
                #print(startEnd)
                st = int(float(startEnd[0].strip(' ')) * 60)
                en = int(float(startEnd[1].strip(' ')) * 60)
                segMap[st:en] = 1.0
        return segMap
    
# Function to get all colums for segannodf
def getAllCols(dfOuranno,serial_id,folderName):
    LOWESSWINDOW = 3.0
    OUTLIERSD = 2.5
    record_name = dfOuranno.loc[serial_id,'RecordNum'] 
    flname = folderName+str(record_name)+'n.hea'
    recname = folderName+str(record_name)+'n'       
    [samples,R,S,H,firstline] = getIndexOfRRSpO2HR(flname)
    rec =  wfdb.io.rdrecord(str(recname))
    xrr = rec.p_signal[:,R]
    xspo2 = rec.p_signal[:,S]
    ...
    TOTAL_LEN = len(xrr)
    t = np.arange(0,TOTAL_LEN,1)
    segMap = getSegmentationMap(dfOuranno,serial_id,TOTAL_LEN)
    
    [rrR,tr] = fillNAzero(xrr,t)
    [spo2R,tr] = fillNAzero(xspo2,t)
    [hrR,tr] = fillNAzero(xhr,t)
    
    
    [xrrnew,trrnew] = outlierRejector(xrr,t,OUTLIERSD)
    [xspo2new,tspo2new] = outlierRejector(xspo2,t,OUTLIERSD)
    ...
    #print(len(HRraw))
    RRlowess = zrrnew[:,1]
    SpO2lowess = zspo2new[:,1]
    HRlowess = zhrnew[:,1]
    Label = segMap
    return RRraw,SpO2raw,HRraw,RRlowess,SpO2lowess,HRlowess,Label